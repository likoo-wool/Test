...
let taskmap = new Map();
let articleidarr = [];
let TokenArr = ['DyaNsFhj9IAJaWRW4a_zPd_fiFZPwBSp'];
!(async () => {
    await all();
})()
    .catch((e) => {
...
async function all() {
    ...
    for (let i = 0; i < TokenArr.length; i++) {
        ...
        Token=TokenArr[i];
        await DailySign();
        await $.wait(randomNum(100, 200));
        await DailyCheck();
        await $.wait(randomNum(100, 200));
        await ArticelsList();
        for (let i = 0; i < articleidarr.length; i++) {
            await ArticleFinish(articleidarr[i]);
            await $.wait(randomNum(300, 400));
        }

        for (let i = 0; i < 5; i++) {
            await ArticleShare();
            await $.wait(randomNum(0, 500));
        }
        for (let i = 0; i < 3; i++) {
            await TaskAD(8);
            await $.wait(randomNum(100, 129));
            await TaskAD(9);
            await $.wait(randomNum(120, 119));
            await TaskAD(10);
            await $.wait(randomNum(110, 109));
            await TaskAD(11);
        }
    }
}
